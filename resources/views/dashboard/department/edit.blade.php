<!-- resources/views/dashboard/department/forms.blade.php -->
        ...
        <div class="space-y-6">
            ...
            <select name="head" id="head" required
              ...
              @foreach ($headOfDepartment as $head)
                <option value="{{ $head->id }}"
                  {{ old('head', $department->head ?? '') == $head->id ? 'selected' : '' }}>
                  {{ $head->name }}
                </option>
              @endforeach
            </select>
          ...
          <div>
            ...
            <textarea name="description" id="description" rows="4"
              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-gray-300">{{ old('description', $department->description ?? '') }}</textarea>
            @error('description')
              <span class="text-sm text-red-600 dark:text-red-400">{{ $message }}</span>
            @enderror
          </div>
          ...
          <div class="flex justify-end">
            <x-primary-button>{{ isset($department) ? 'Update' : 'Save' }}</x-primary-button>
          </div>
